...
const MOCK_DATA: AnalysisResult = {
  ...
  quiz: [
    ...
    {
      id: 4,
      question: "Which formula describes the efficiency improvement in HERIC?",
      options: ["Efficiency = P_out / P_in", "V_leakage = V_dc / 2", "I_rms = I_max / sqrt(2)", "P_loss = I^2 * R"],
      correctAnswer: "V_leakage = V_dc / 2",
      socraticHint: "Focus on how the zero state splits the voltage stress.",
      type: 'Formula'
    },
    {
      id: 5,
      question: "If S5 fails, what happens to the zero state?",
      options: ["The circuit explodes", "Leakage current increases", "The load stops working", "The battery charges faster"],
      correctAnswer: "Leakage current increases",
      socraticHint: "The 'wicket-keeper' is gone, so extra 'runs' (leakage) will escape.",
      type: 'Applied Logic'
    },
    {
      id: 6,
      question: "What is the total number of power switches in a basic HERIC topology?",
      options: ["4", "5", "6", "8"],
      correctAnswer: "6",
      socraticHint: "Count the main H-bridge switches plus the freewheeling pair.",
      type: 'Formula'
    }
  ],
  flashcards: [
...
export const analyzeImage = async (
  ...
  // PRIORITY: Settings Key -> LocalStorage -> Hardcoded Fallback     AIzaSyAyYoturduY2cLl4Zg5z0dwA0A1L2neZz8  AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM
  const API_KEY = settings.apiKey || localStorage.getItem('gemini_api_key') || "AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl";
...
/**
 * Fetch a fresh, advanced quiz from Gemini for a specific topic.
 */
export const getAdvancedQuiz = async (topic: string, userApiKey?: string): Promise<QuizQuestion[]> => {
  const API_KEY = userApiKey || localStorage.getItem('gemini_api_key') || "AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl";

  //    AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl                 AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM
  ...
  try {
    ...
    throw new Error("Could not fetch advanced questions. Please try again.");
  }
};

export const defineWord = async (word: string, userApiKey?: string): Promise<string> => {   // AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl      AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM
  const API_KEY = userApiKey || localStorage.getItem('gemini_api_key') || "AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl";
...
export const generateHint = async (
  ...
  topic: string,
  userApiKey?: string
): Promise<string> => {                                          //          AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl                     AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM
  const API_KEY = userApiKey || localStorage.getItem('gemini_api_key') || "AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl";
...
export const expandMindMapNode = async (
  ...
  const API_KEY = userApiKey || localStorage.getItem('gemini_api_key') || "AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl";   // AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM
...
export const getVivaResponse = async (
  ...
  if (!API_KEY) return "Friend, please set your API key in settings first!";
...
export const getDebateResponse = async (
  ...
  const API_KEY = apiKey || localStorage.getItem('gemini_api_key') || "AlzaSyBrcrESz7Up580BxCRscMTdg7yoOgoWeCl";
...
//   // PRIORITY: Settings Key -> LocalStorage -> Hardcoded Fallback     AIzaSyAyYoturduY2cLl4Zg5z0dwA0A1L2neZz8  AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM
//   const API_KEY = settings.apiKey || localStorage.getItem('gemini_api_key') || "AIzaSyAyYoturduY2cLl4Zg5z0dwA0A1L2neZz8";
...
//  */
// export const getAdvancedQuiz = async (topic: string, userApiKey?: string): Promise<QuizQuestion[]> => {
//   const API_KEY = userApiKey || localStorage.getItem('gemini_api_key') || "AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM";
...
// export const defineWord = async (word: string, userApiKey?: string): Promise<string> => {
//   const API_KEY = userApiKey || localStorage.getItem('gemini_api_key') || "AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM";
...
//   apiKey: string
// ): Promise<string> => {
//   const API_KEY = apiKey || localStorage.getItem('gemini_api_key') || "AIzaSyDNHc4FhihViW0nDjm6j1wbH9k3X-PQeJM";
